...
impl Progress {
    pub fn new(weight: u64) -> Progress {
        let stuff = _P { parent: None, progress: 0.0, weight, children: Vec::new(), tracker: None };

        Progress { stuff: Rc::new(RefCell::new(stuff)) }
    }
    ...
    pub fn append(&mut self, p: &[&Progress]) {
        ...
        if t_s.children.len() == 0  && t_s.weight > 0 {
            ...
            let c = _P { parent: Some(self.stuff.clone()), progress: t_s.progress, weight: t_s.weight, children: Vec::new(), tracker: None };
